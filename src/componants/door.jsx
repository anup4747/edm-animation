...
export default function Door({ position = [0, 0, 0], isOpen }) {
  ...
  useFrame((state, delta) => {
    if (isOpen && leftRef.current && rightRef.current) {
      leftRef.current.position.x -= delta * 1.000001; // Slightly faster for impact
      rightRef.current.position.x += delta * 1.000001;
    }
  });
  ...
  return (
    <group position={position}>
      ...
      <mesh ref={rightRef} position={[0.225, 0, 0.001]} castShadow>
        ...
        {/* Red Glowing Edges */}
        <Edges scale={1} threshold={15} color="#ff0000" linewidth={5} />
      </mesh>
